...
export class DoctorController {
  ...
  async approveDoctor(req: Request, res: Response): Promise<void> {
    try {
      ...
      const approvedDoctor = await this.doctorService.approveDoctor(doctorId);
      if (!approvedDoctor) {
        sendError(res, HttpStatus.NotFound, MessageConstants.DOCTOR_NOT_FOUND);
        return;
      }
      ...
      sendResponse(
        res,
        HttpStatus.OK,
        MessageConstants.DOCTOR_APPROVED,
        approvedDoctor
      );
    } catch (error: any) {
      sendError(
