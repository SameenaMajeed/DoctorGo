...
export class ChatRepository {
  async getUsersWhoMessaged(doctorId: string): Promise<string[]> {
    try {
      ...
      const userIds = await messageModel
  ...
  async getUserDetails(userIds: string[]): Promise<
    {
      id: string;
      name: string;
      mobile_no?: string;
  ...
  async getDoctorDetails(doctorIds: string[]): Promise<
    {
      id: string;
      name: string;
      profilePicture?: string;
  ...
  async findMessagesByUserAndDoctor(
    userId: string,
    doctorId: string
  ): Promise<any[]> {
    try {
